...
exports.deleteSymbolsFor = deleteSymbolsFor;
exports.deleteImportsFor = deleteImportsFor;
...
async function ensureSchema(session) {
    const stmts = [
        ...
        `CREATE INDEX IF NOT EXISTS FOR (f:File) ON (f.ext)`,
        `CREATE INDEX IF NOT EXISTS FOR (f:File) ON (f.isTest)`,
        `CREATE INDEX IF NOT EXISTS FOR (f:File) ON (f.isEntryPoint)`,
        `CREATE INDEX IF NOT EXISTS FOR (f:File) ON (f.language)`,
        `CREATE INDEX IF NOT EXISTS FOR (f:File) ON (f.external)`,
...
async function ensureProjectRoot(session, project, root) {
    await session.run(`MERGE (p:Project {name: $name}) SET p.root = $root
     MERGE (d:Folder {path: "."}) SET d.name = $name
     MERGE (p)-[:CONTAINS]->(d)`, { name: project, root });
}
// ── folders ───────────────────────────────────────────────────────────────────
async function writeFolders(session, folderPaths, project) {
    if (!folderPaths.length)
        return;
    await session.run(`UNWIND $folders AS fp
     MERGE (d:Folder {path: fp})
     SET d.name = CASE WHEN fp = "." THEN $proj ELSE last(split(fp, "/")) END`, { folders: folderPaths, proj: project });
    await session.run(`UNWIND $folders AS fp
     WITH fp WHERE fp <> "."
     WITH fp,
     ...
     MERGE (a)-[:CONTAINS]->(b)`, { folders: folderPaths });
}
// ── files ─────────────────────────────────────────────────────────────────────
async function writeFiles(session, files) {
    if (!files.length)
        return;
    await session.run(`UNWIND $files AS f
     ...
     MERGE (d)-[:CONTAINS]->(file)`, {
        files: files.map((f) => ({
            ...
            lineCount: (0, driver_js_1.toInt)(f.lineCount),
        })),
    });
}
// ── symbols ───────────────────────────────────────────────────────────────────
async function writeSymbols(session, symbols) {
    if (!symbols.length)
        return;
...
// ── imports ───────────────────────────────────────────────────────────────────
async function writeImports(session, rawImports) {
...
// ── calls ─────────────────────────────────────────────────────────────────────
async function writeCalls(session, calls) {
    if (!calls.length)
        return;
    await session.run(`UNWIND $calls AS c
     ...
     MERGE (a)-[:CALLS]->(b)`, {
        calls: calls.map((c) => ({
            from: { ...c.from, startLine: (0, driver_js_1.toInt)(c.from.startLine) },
            to: { ...c.to, startLine: (0, driver_js_1.toInt)(c.to.startLine) },
        })),
    });
}
// ── symbol → symbol edges ─────────────────────────────────────────────────────
...
async function writePlanItems(session, items) {
    if (!items.length)
        return;
    await session.run(`UNWIND $items AS p
     ...
     MATCH (t) WHERE (t:File OR t:Folder) AND t.path = tp
     MERGE (pi)-[:TARGETS]->(t)`, { items: items.map((p) => ({ ...p, index: (0, driver_js_1.toInt)(p.index) })) });
}
async function writeDecisions(session, items) {
    if (!items.length)
        return;
    await session.run(`UNWIND $items AS x
     ...
     MATCH (t) WHERE (t:File OR t:Folder) AND t.path = tp
     MERGE (dec)-[:TARGETS]->(t)`, { items: items.map((d) => ({ ...d, index: (0, driver_js_1.toInt)(d.index) })) });
}
async function writeConstraints(session, items) {
    if (!items.length)
        return;
    await session.run(`UNWIND $items AS x
     ...
     UNWIND x.targetPaths AS tp
     MATCH (t) WHERE (t:File OR t:Folder) AND t.path = tp
     MERGE (c)-[:TARGETS]->(t)`, { items: items.map((c) => ({ ...c, index: (0, driver_js_1.toInt)(c.index) })) });
}
// ── delete helpers (incremental) ──────────────────────────────────────────────
async function deleteFile(session, p) {
...
async function deleteSymbolsFor(session, p) {
    await session.run(`MATCH (f:File {path:$p})-[:DEFINES]->(s:Symbol) DETACH DELETE s`, { p });
}
...
async function deleteCallsTouching(session, paths) {
    if (!paths.length)
        return;
    await session.run(`UNWIND $paths AS p
     MATCH (a:Symbol)-[r:CALLS]->(b:Symbol)
     WHERE a.file = p OR b.file = p DELETE r`, { paths });
}
async function deleteDoc(session, p) {
